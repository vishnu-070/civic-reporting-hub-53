...
Current context: You're assisting a citizen on the RTIRS platform.`
  ...
  const chatWindowClasses = isMobile 
    ? "fixed inset-4 z-50 animate-scale-in"
    : "fixed bottom-6 right-6 z-50 w-80 h-96 animate-scale-in";

  const chatButtonClasses = isMobile
    ? "h-12 w-12 rounded-full bg-blue-600 hover:bg-blue-700 shadow-lg"
    : "h-14 w-14 rounded-full bg-blue-600 hover:bg-blue-700 shadow-lg animate-bounce";
            ...
            <CardContent className="flex-1 flex flex-col p-0">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {messages.map((message) => (
                  ...
                  >
                    <div
                      className={`max-w-xs px-3 py-2 rounded-lg ${isMobile ? 'text-xs' : 'text-sm'} ${
                        message.isUser
                  ...
                  <div className="space-y-2">
                    ...
                    {predefinedMessages.map((message, index) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className={`w-full ${isMobile ? 'text-xs h-auto py-2 px-2' : 'text-xs h-auto py-2 px-2'} text-left justify-start whitespace-normal`}
